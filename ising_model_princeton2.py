...
class Ising_lattice:
   ...
   def choose_level(self,data,classes):
      label=self._label
      if label not in [np.min(classes),np.max(classes)]:
   ...
   def cond_spin_flip(self,i,j,T): 
      ...
      if (dE<0.0 or (T>0.0 and (np.random.random()<np.exp(-dE/T)))):
          return dE # stay at the lattice value (i,j) with the probability
      self.spin_flip(i,j) # update lattice value i,j and the corresponding energy
      return 0
