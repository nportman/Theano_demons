#M-H algorithm with time-dependent transitional probabilities
"""
Created on Wed, March 02nd, 2016
...
def choose_level2(en, T, classes, deck, freq):
    if deck==np.min(classes):
        deck=deck+1
    elif deck==np.max(classes):
        deck=deck-1
        ...
        En_A1=en[deck-1]
        En_A2=en[deck+1]
        p_A1B=np.exp(-(En_B-En_A1)/T)*prob_A1
        #print p_A1B
        p_A2B=np.exp(-(En_B-En_A2)/T)*prob_A2
        #print p_A2B
        s=p_A1B+p_A2B+prob_B
...
def get_MH_sampled_IDs(data,en,classes,freq):
    ...
    for T in temp_val:
        ...
        for n in range(N_seeds):
            ...
            for k in range(iters): # num of MH steps
                ...
                label=choose_level2(en,T,classes,deck,freq) # next candidate from the label group
...
def main():
    ...
    train_res=get_MH_sampled_IDs(train2,train_en,train_classes,freq) 
    H=[]
    ...
    val=count.values()    
    freq2=np.array(val)/np.float(len(H)) # estimated fdrequencies of the ditribution   
    print "energy values:", count.keys()
    print "frequencies:", freq2
    histogr(H)    
